...
def run_content_based_recommendation(user_id):
    ...
    # get movieId list
    movies_id_list = ratings["movieId"].iloc[history_indices].tolist()
    ...
    for movie_id in movies_id_list:
        ...
        genres_based_recommendation_indices = get_recommendations_idx(
            movie_id, 0.9, cosine_sim
        )
        recommendation_indices = list(
            set().union(recommendation_indices, genres_based_recommendation_indices)
        )

        # get title based recommendation indices & union list
        title_based_recommendation_indices = get_recommendations_idx(
            movie_id, 0.9, cosine_sim_2
        )
        recommendation_indices = list(
            set().union(recommendation_indices, title_based_recommendation_indices)
        )

    # remove already watched movie
    recommendation_indices = set(recommendation_indices) - set(history_indices)

    return list(recommendation_indices)
